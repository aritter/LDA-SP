 ...
 *Manually modified to implement a parallel sampler (using MPI) similar to "Approximate Distributed LDA" (Newman et. al. 2009)

HBC is a freely available compiler for statistical models.  This generated
code can be built using the following command:

  mpicc -mpe=log -lm stats.c samplib.c link_lda_300_mpi.c -o link_lda_300_mpi.out
  mpiexec -n 20 `pwd`/link_lda_300_mpi.out > test.samples
...
void resample_post_theta(int K, int* N, double** post_theta, int** z1, int** z2, int s, int e) {
  int d_13;
  double* tmpSP10;
  int n_4;
  double* tmpSP11;
...
void resample_post_beta1(int K, int* N, int V1, int** arg1, double** post_beta1, int** z1, int s, int e) {
  int k_11;
  double* tmpSP6;
  int d_2;
  int n_110;
...
void resample_post_beta2(int K, int* N, int V2, int** arg2, double** post_beta2, int** z2, int s, int e) {
  int k_12;
  double* tmpSP8;
  int d_3;
  int n_111;
...
void resample_z1(int* N, double alpha, int** arg1, double eta, double** post_beta1, double** post_beta1_global, double** post_theta, int** z1, int K, int V1, int s, int e) {
  int d_14;
  int n_23;
  ...
  for (d_14=s; d_14<=e; d_14++) {
    for (n_23=1; n_23<=N[d_14-1]; n_23++) {
      ...
      post_beta1_global[z1[d_14-1][n_23-1]-1][(V1) + (1)-1] += (0.0) - ((1.0) * ((((z1[d_14-1][n_23-1]) == (z1[d_14-1][n_23-1])) ? 1 : 0)));
      post_beta1_global[z1[d_14-1][n_23-1]-1][arg1[d_14-1][n_23-1]-1] += (0.0) - ((1.0) * ((((z1[d_14-1][n_23-1]) == (z1[d_14-1][n_23-1])) ? 1 : 0)));
      ...
      for (tmp_idx_z1_2=1; tmp_idx_z1_2<=K; tmp_idx_z1_2++) {
        tmp_post_z1_2[tmp_idx_z1_2-1] = (ldf_Mult_smooth(0, eta, arg1[d_14-1][n_23-1], post_beta1_global[tmp_idx_z1_2-1], 1, V1)) + (ldf_Mult_smooth(0, alpha, tmp_idx_z1_2, post_theta[d_14-1], 1, K));
      }
      ...
      post_beta1_global[z1[d_14-1][n_23-1]-1][(V1) + (1)-1] += (1.0) * ((((z1[d_14-1][n_23-1]) == (z1[d_14-1][n_23-1])) ? 1 : 0));
      post_beta1_global[z1[d_14-1][n_23-1]-1][arg1[d_14-1][n_23-1]-1] += (1.0) * ((((z1[d_14-1][n_23-1]) == (z1[d_14-1][n_23-1])) ? 1 : 0));
...
void resample_z2(int* N, double alpha, int** arg2, double eta, double** post_beta2, double** post_beta2_global, double** post_theta, int** z2, int K, int V2, int s, int e) {
  int d_15;
  int n_24;
  ...
  for (d_15=s; d_15<=e; d_15++) {
    for (n_24=1; n_24<=N[d_15-1]; n_24++) {
      ...
      post_beta2_global[z2[d_15-1][n_24-1]-1][(V2) + (1)-1] += (0.0) - ((1.0) * ((((z2[d_15-1][n_24-1]) == (z2[d_15-1][n_24-1])) ? 1 : 0)));
      post_beta2_global[z2[d_15-1][n_24-1]-1][arg2[d_15-1][n_24-1]-1] += (0.0) - ((1.0) * ((((z2[d_15-1][n_24-1]) == (z2[d_15-1][n_24-1])) ? 1 : 0)));
      ...
      for (tmp_idx_z2_2=1; tmp_idx_z2_2<=K; tmp_idx_z2_2++) {
        tmp_post_z2_2[tmp_idx_z2_2-1] = (ldf_Mult_smooth(0, eta, arg2[d_15-1][n_24-1], post_beta2_global[tmp_idx_z2_2-1], 1, V2)) + (ldf_Mult_smooth(0, alpha, tmp_idx_z2_2, post_theta[d_15-1], 1, K));
      }
      ...
      post_beta2_global[z2[d_15-1][n_24-1]-1][(V2) + (1)-1] += (1.0) * ((((z2[d_15-1][n_24-1]) == (z2[d_15-1][n_24-1])) ? 1 : 0));
      post_beta2_global[z2[d_15-1][n_24-1]-1][arg2[d_15-1][n_24-1]-1] += (1.0) * ((((z2[d_15-1][n_24-1]) == (z2[d_15-1][n_24-1])) ? 1 : 0));
...
void initialize_z1(int** z1, int* N, int K, int s, int e) {
  int d_14;
  int n_23;
  int dvv_loop_var_1;
  int dvv_loop_var_2;
  for (dvv_loop_var_1=s; dvv_loop_var_1<=e; dvv_loop_var_1++) {
...
void initialize_z2(int** z2, int* N, int K, int s, int e) {
  int d_15;
  int n_24;
  int dvv_loop_var_1;
  int dvv_loop_var_2;
...
void initialize_post_theta(double** post_theta, int K, int* N, int** z1, int** z2, int s, int e) {
  int dvv_loop_var_1;
  int dvv_loop_var_2;
  for (dvv_loop_var_1=s; dvv_loop_var_1<=e; dvv_loop_var_1++) {
  ...
  resample_post_theta(K, N, post_theta, z1, z2, s, e);
}

void initialize_post_beta1(double** post_beta1, int K, int* N, int V1, int** arg1, int** z1, int s, int e) {
  int dvv_loop_var_1;
  int dvv_loop_var_2;
  for (dvv_loop_var_1=1; dvv_loop_var_1<=K; dvv_loop_var_1++) {
  ...
  resample_post_beta1(K, N, V1, arg1, post_beta1, z1, s, e);
}

void initialize_post_beta2(double** post_beta2, int K, int* N, int V2, int** arg2, int** z2, int s, int e) {
  int dvv_loop_var_1;
  int dvv_loop_var_2;
  for (dvv_loop_var_1=1; dvv_loop_var_1<=K; dvv_loop_var_1++) {
  ...
  resample_post_beta2(K, N, V2, arg2, post_beta2, z2, s, e);
}
...
void update_global_counts(double **post_beta, double **post_beta_global, int K, int V, int myid, int numprocs)
{
  int k;

  for(k=0; k<K; k++) {
    //fprintf(stderr, "updating global counts for topic %d of %d\n", k, K);
    //fflush(stderr);
    MPI_Allreduce(post_beta[k], post_beta_global[k], V+1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  }
}

/*
 * Decomposition based on number of words in each document
 */
void MY_Decomp1d(int n, int size, int rank, int *N, int D, int *s, int *e)
{
  int nWords = 0;
  int wpp;   //Words per processor
  int i;
  int p;
  int pstart;
  int pend;
  ...
  wpp = floor(nWords/size);

  i = 0;
  nWords = 0;
  for(p=0; p<size; p++) {
    pstart = i;
    //Always add at least one document per process (e.g. some docs can be huge)...
    nWords += N[i];
    i += 1;
    while(nWords+N[i] < wpp*(p+1) && i < D) {	//Only add the next document if it won't put us over...
      nWords += N[i];
      i += 1;
    }
    pend = i;
...
void gather_z(int **z, int **z_global, int D, int *N, int myid, int numprocs)
{
  int p;
  int d;
  int s;
  ...
  //Copy local part
  if(myid == 0) {
    MY_Decomp1d(D, numprocs, myid, N, D, &s, &e);
    for(d=s; d<=e; d++) {
      memcpy(z_global[d-1], z[d-1], (N[d-1]*sizeof(int)));
    }
  }

  //Collect samples from other nodes
  for(p=1; p<numprocs; p++) {
    //Get the starting and ending documents assigned to proccessor p
    MY_Decomp1d(D, numprocs, p, N, D, &s, &e);

    //Send sample for each z associated with p to proccesor 0
    for(d=s; d<=e; d++) {
      if(myid == p) {
	MPI_Send(z[d-1],        N[d-1], MPI_INT, 0, 0, MPI_COMM_WORLD);
      }
      if(myid == 0) {
	MPI_Recv(z_global[d-1], N[d-1], MPI_INT, p, 0, MPI_COMM_WORLD, &status);
      }
    }
...
int main(int ARGC, char *ARGV[]) {
  ...
  double eta;
  double** post_beta1;
  double** post_beta1_global;
  double** post_beta2;
  double** post_beta2_global;
  double** post_theta;
  int** z1;
  int** z1_global;
  int** z2;
  ...
  if(ARGC != 6) {
    fprintf(stderr, "usage: %s <arg1> <arg2> <nIter> <dumpInterval> <syncInterval>\n", ARGV[0]);
    exit(1);
  }

  nIter = atoi(ARGV[3]);
  dumpInterval = atoi(ARGV[4]);
  syncInterval = atoi(ARGV[5]);
  ...
  /* Figure out which documents are assigned to this processor */
  MY_Decomp1d(D, numprocs, myid, N, D, &s, &e);
  //MPE_Decomp1d(D, numprocs, myid, &s, &e);
  fprintf(stderr, "\n%d: start=%d, end=%d\n", myid, s, e);
  fflush(stderr);  
  ...
  post_beta1_global = (double**) malloc(sizeof(double*) * (1+(K)-(1)));
  for (malloc_dim_1=1; malloc_dim_1<=K; malloc_dim_1++) {
    post_beta1_global[malloc_dim_1-1] = (double*) malloc(sizeof(double) * (1+((V1) + (1))-(1)));
  }
  ...
  post_beta2_global = (double**) malloc(sizeof(double*) * (1+(K)-(1)));
  for (malloc_dim_1=1; malloc_dim_1<=K; malloc_dim_1++) {
    post_beta2_global[malloc_dim_1-1] = (double*) malloc(sizeof(double) * (1+((V2) + (1))-(1)));
  }
  ...
  z1_global = (int**) malloc(sizeof(int*) * (1+(D)-(1)));
  for (malloc_dim_1=1; malloc_dim_1<=D; malloc_dim_1++) {
    z1_global[malloc_dim_1-1] = (int*) malloc(sizeof(int) * (1+((N[malloc_dim_1-1]) + (1))-(1)));
  }
  ...
  z2_global = (int**) malloc(sizeof(int*) * (1+(D)-(1)));
  for (malloc_dim_1=1; malloc_dim_1<=D; malloc_dim_1++) {
    z2_global[malloc_dim_1-1] = (int*) malloc(sizeof(int) * (1+((N[malloc_dim_1-1]) + (1))-(1)));
  }

  fprintf(stderr, "Initializing variables...\n");
  fflush(stderr);
  initialize_z1(z1, N, K, s, e);
  initialize_z2(z2, N, K, s, e);
  initialize_post_theta(post_theta, K, N, z1, z2, s, e);
  initialize_post_beta1(post_beta1, K, N, V1, arg1, z1, s, e);
  initialize_post_beta2(post_beta2, K, N, V2, arg2, z2, s, e);

  for (iter=1; iter<=nIter; iter++) {
    fprintf(stderr, "proc %d iter %d\n", myid, iter);
    fflush(stderr);

    /*
     * reduce/broadcast
     * Note; we need to synchronize on the first iteration
     */
    if((iter % syncInterval) == 0 || iter == 1) {
      update_global_counts(post_beta1, post_beta1_global, K, V1, myid, numprocs);
      update_global_counts(post_beta2, post_beta2_global, K, V2, myid, numprocs);
    }

    resample_z1(N, alpha, arg1, eta, post_beta1, post_beta1_global, post_theta, z1, K, V1, s, e);
    resample_z2(N, alpha, arg2, eta, post_beta2, post_beta2_global, post_theta, z2, K, V2, s, e);

    //TODO: update compute_log_poster (not highest priority)
    /*
    fprintf(stderr, "%d: computing loglik\n", myid);
    fflush(stderr);
    ...
    */
    if ((iter % dumpInterval) == 0) {
      ...
      fprintf(stderr, "%d: gathering z\n", myid);
      fflush(stderr);
      gather_z(z1, z1_global, D, N, myid, numprocs);
      gather_z(z2, z2_global, D, N, myid, numprocs);

      if(myid == 0) {
	dump_z1(D, N, z1_global);
	dump_z2(D, N, z2_global);
      }
    }
  }
